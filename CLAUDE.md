...
- **Datengetriebenes Projekt-System (Stand Juli 2026):** Eine zentrale Quelle `js/projects.js` (`PROJECT_ORDER` + `PROJECTS_DATA`: title/meta/role/year/sub/link/facts/body/note + `images[]` + `video`). Sowohl das Selected-Work-Raster (index.html) als auch EINE Template-Detailseite `work/project.html?p=<slug>` rendern daraus. Kein Hand-Anlegen einzelner HTML-Seiten mehr — neues Projekt = Ordner in assets/ + Eintrag in projects.js. Pfade in projects.js sind root-relativ (`assets/…`); project.html stellt beim Rendern `../` voran. Video-Projekte: `video`-Feld (committetes `*-web.mp4`) → Video-Hero + „▶ Film"-Badge im Raster; EHS-Arbeiten bekommen `note` (EHS-Media-Hinweis). Beim Bilder-Sync werden die `images[]`/`video` in projects.js neu aus den Ordnern generiert.
- Projekt-Unterseiten teilen sich `css/style.css` (Design-Tokens, Header/Menu/Footer, Sidenotes + Projekt-Detail-Layout: .proj-hero/.proj-body/.proj-gallery/.proj-facts/.proj-next). Achtung auch hier: .wrap-Sektionen NIE mit padding-Kurzform — immer padding-top/-bottom, sonst stirbt der Seitenrand.
